...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true)
        const [latestBooksData, bestBooksData, reviewsData] = await Promise.all([
          api.getLatestBooks(4),
          api.getLatestBooks(100), // Получаем больше книг для сортировки по рейтингу
          api.getReviews(0, 3)
        ])
        setLatestBooks(latestBooksData)
        // Сортируем книги по рейтингу и берем топ-12
        const sortedByRating = bestBooksData
          .sort((a, b) => (b.average_rating || 0) - (a.average_rating || 0))
          .slice(0, 12)
        setBestBooks(sortedByRating)
