...
export default function PublisherCatalogPage({ params }: { params: { slug: string } }) {
  ...
  const getPublisherName = (slug: string) => {
    const publishers: { [key: string]: string } = {
      ast: "АСТ",
      ripol: "РИПОЛ",
      rosman: "Росмэн",
      triumph: "Триумф",
      all: "Все издательства",
    }
    return publishers[slug] || "Каталог"
  }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center mb-6">
          ...
          <div>
            <p className="text-sm text-gray-500">Все книги / Издательство/{getPublisherName(params.slug)}</p>
            <h1 className="text-2xl font-bold text-gray-900">{getPublisherName(params.slug)}</h1>
          </div>
        </div>
