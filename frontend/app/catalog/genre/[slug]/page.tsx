...
export default function GenreCatalogPage({ params }: { params: { slug: string } }) {
  ...
  const getGenreName = (slug: string) => {
    const genres: { [key: string]: string } = {
      detective: "Детектив",
      fantasy: "Фантастика",
      "fantasy-genre": "Фэнтези",
      romance: "Романы",
      all: "Все жанры",
    }
    return genres[slug] || "Каталог"
  }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center mb-6">
          ...
          <div>
            <p className="text-sm text-gray-500">Все книги / Жанр/{getGenreName(params.slug)}</p>
            <h1 className="text-2xl font-bold text-gray-900">{getGenreName(params.slug)}</h1>
          </div>
        </div>
